...
\hspace{1cm}\textit{What about in a bipartite graph? In a bipartite graph, edges are formed
    between the top and bottom vertices. Thus, we can count the node degrees on either
    row, sum them up, and that will give us our edges. If $V$ is our number of vertices, then
    we can form, at most, $V/2$ connections per node, and still have a bipartite graph.
    if we sum up this max per every node in either (but not both) row(s), we would sum the node
